...
- [ ] G6: Live AvailableModels catalog matrix (text + bash/read/write) for every non-skipped id
      ...
      EVIDENCE: pending live renewer on the run host. `--allow-missing-creds` is the no-secrets CI path. Matrix retries HTTP 502/504, empty-body, and incomplete-tool. Auto `default` rewrites to bare sand-default; `gemini-3-flash` / `gemini-3-flash[]` rewrite requestedModel to bare `gemini-3.8-flash`.
