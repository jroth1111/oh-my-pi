...
Drop abandons a task but keeps it incomplete for the stop reminder—only the user can continue past drops. External waits (user/agent/service): block with optional reason suppresses stop reminder, starts next pending. Unblock when actionable; append a clearing task for agent-actionable blocker.
