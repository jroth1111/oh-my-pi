...
it("sends Azure Responses structured-output fields and continuation IDs in serialized JSON", async () => {
	...
	await streamAzureOpenAIResponses(
		...
		{ messages: [{ role: "user", content: "fixture", timestamp: 0 }] },
		{
			...
			responseFormat: {
				type: "json_schema",
				json_schema: { name: "answer", description: "contract", strict: false, schema: { type: "object" } },
			},
